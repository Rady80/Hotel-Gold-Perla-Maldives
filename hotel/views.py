...
@login_required(login_url='login')  # Zajistí, že uživatel je přihlášen
def home(request):
    # Ověření, zda uživatel patří do nějaké skupiny
    if request.user.groups.exists():
       role = str(request.user.groups.first())
    else:
         role = "No Group"  # Nastavte výchozí hodnotu pro uživatele bez skupiny
         context = {"role": role,}
